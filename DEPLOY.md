...
4. Переменные окружения (Environment variables):
   - `KIE_API_KEY` = `<новый ключ от сервиса Kie.ai>`  ← без него чат-бот не отвечает
...
## Ключ KIE_API_KEY
Это доступ к сервису **Kie.ai** (шлюз к AI-моделям; в коде используется модель
`gemini-3-flash`). Новый ключ создаётся в личном кабинете на kie.ai.
Нужен только для работы чат-бота «Макс»; сам сайт откроется и без него.
