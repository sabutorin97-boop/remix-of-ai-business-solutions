...
(серверный роут `/api/chat`, требует ключ `KIE_API_KEY`).
...
4. Переменные окружения (Environment variables):
   - `KIE_API_KEY` = `<ключ от сервиса kie.ai>`  ← без него чат-бот не отвечает
...
## Ключ KIE_API_KEY
Это доступ к сервису **kie.ai** (в коде используется модель `gemini-3-flash`).
Ключ создаётся в личном кабинете на kie.ai → api-key. Нужен только для
работы чат-бота «Макс»; сам сайт откроется и без него.

OpenRouter (Claude Haiku 4.5, до этого DeepSeek V4 Flash) тестировался как
альтернатива, но с продовых IP Timeweb Cloud Apps OpenRouter отвечает 403
"Access denied by security policy" на каждый запрос (WAF-блок Cloudflare,
тот же класс проблемы, что и в проекте Threads automation) — вернулись
на kie.ai, он с этих IP работает.
...
- `KIE_API_KEY` не задан (на сборку не влияет, но чат-бот вернёт 500 при обращении);
