...
используется модель `deepseek/deepseek-v4-flash` — дешёвая и достаточно
умная для короткого консультационного бота). Ключ создаётся в личном
кабинете на openrouter.ai → раздел «Keys». Нужен только для работы чат-бота
«Макс»; сам сайт откроется и без него.
Ответы бота ограничены 250 символами и темой услуг AI-Profigrup — это задано
в системном промпте `src/routes/api/chat.ts`, менять модель/лимит можно там.
