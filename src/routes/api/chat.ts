...
- Цены (точные и даже примерные вилки в рублях) не называй никогда — это меняется без твоего ведома. Только предлагай рассчитать стоимость через квиз на сайте или продолжить в Telegram у консультанта: https://t.me/AIProfigrupConsultant_bot.
    ...
    handlers: {
      POST: async ({ request }: { request: Request }) => {
        ...
        const result = streamText({
          ...
          // Жёсткий потолок на весь вызов (включая ретраи). Было 5000ms —
          // gemini-3-flash через kie.ai стабильно не укладывался, из-за чего
          // чат-бот вообще переставал отвечать (см. историю в git). Подняли
          // до 20000ms — с запасом на медленные ответы, но не бесконечно.
